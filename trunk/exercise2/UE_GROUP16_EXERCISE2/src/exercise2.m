...
%   are stored in OUTPUT_DIRECTORY.
%
%   ABGABE2(INPUT_DIRECTORY, OUTPUT_DIRECTORY, START_FRAME, END_FRAME) only
...
function exercise2(input_directory, output_directory, start_frame, end_frame)
    ...
    for i = 1:1:video.BUFFER_FRAMES
        ...
        video.frame(i).frame_nr             = -1;     % frame number of this frame (CAN CHANGE IF ADDITIONAL FRAMES ARE INSERTED/REMOVED BY A FILTER/EFFECT)
    ...
    % OPTIONAL PARAMETERS
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    selected_input_frames = ones(1,numel(video.input_files));
    if (exist('start_frame'))
        selected_input_frames(1:start_frame-1) = 0;     
    end
    
    if (exist('end_frame'))
        if (end_frame < numel(video.input_files))
            selected_input_frames(end_frame+1:end) = 0;
        end
    end

    video.input_files = video.input_files(selected_input_frames == 1);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    posFrames = zeros(length(video.input_files));
    [video, posFrames] = effect_add_text(video, {{'../text/scene_text2.png', 6, 2}, ...     
                                   {'../text/scene_text1.png', 1, 3}, ...
                                   {'../text/scene_text1.png', 9, 3}});     % add text frames at position X1 in original input files, text is displayed for Y1 frames
...
%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for frame_counter = 1:numel(video.input_files)+video.BUFFER_FRAMES
...
%             video      = filter_low_framerate(video, source_fps, dest_fps);          

             video = effect_scene_cut(video, 0.61, 23);                  % OPTIONAL
        end

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % APPLY EFFECTS (EXERCISE 2)
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
